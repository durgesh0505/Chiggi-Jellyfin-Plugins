...
public class TraktApi
{
    ...
    public async Task<List<TraktScrobbleResponse>> SendEpisodeStatusUpdateAsync(Episode episode, MediaStatus mediaStatus, TraktUser traktUser, float progressPercent, bool useProviderIds = true)
    {
        ...
        else if (episode.IndexNumber.HasValue)
        {
            ...
            {
                // Fix #258: Guard against null Series reference which occurs in Jellyfin 10.11
                // when episode metadata is partially loaded during playback startup.
                if (episode.Series == null)
                {
                    _logger.LogWarning("Episode {EpisodeName} has a null Series reference - cannot scrobble by season/episode number.", episode.Name);
                    continue;
                }
...
#pragma warning restore CA2007
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            // Fix #266: 409 Conflict means Trakt already has an active scrobble session for this item.
            // This is not an error - treat it as a no-op and return default instead of throwing.
            if (response.StatusCode == HttpStatusCode.Conflict)
            {
                _logger.LogDebug("Received 409 Conflict from Trakt for {Url} - item is already being scrobbled. Ignoring.", url);
                return default(T);
            }
    ...
    private async Task<HttpResponseMessage> RetryHttpRequest(Func<Task<HttpResponseMessage>> function)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            try
            {
                ...
                // Fix #265: HttpStatusCode is not a [Flags] enum. HasFlag() produces wrong results here.
                // Use == for exact status code comparison.
